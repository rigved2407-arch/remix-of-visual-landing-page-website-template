...
const GOLD_COLORS: { id: GoldColor; label: string; swatch: string }[] = [
...
export function JewelryConfiguratorModal() {
  ...
  const handleMetalChange = (id: Metal) => {
    setState((prev) => ({ ...prev, metal: id, goldColor: id !== 'gold' ? null : prev.goldColor }))
  }

  const handleClose = () => {
    closeConfigurator()
    setState({ pieceType: null, metal: null, goldColor: null, notes: '' })
  }
  ...
  const metalLabel = state.metal ? METALS.find((m) => m.id === state.metal)?.label ?? '' : ''
  const goldLabel = state.goldColor ? GOLD_COLORS.find((c) => c.id === state.goldColor)?.label ?? '' : ''
              ...
              <div className="p-8">
                <div className="grid lg:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-8">
                    ...
                    {/* Colour of the Metal (only when gold is selected) */}
                    {state.metal === 'gold' && (
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                          {GOLD_COLORS.map((c) => {
                            ...
                            return (
                              ...
                              >
                                <span className="w-5 h-5 rounded-full shrink-0" style={{ background: c.swatch, boxShadow: '0 0 0 1px rgba(255,255,255,0.15)' }} />
                                <span className="truncate text-xs font-semibold uppercase tracking-[0.1em]">{c.label}</span>
                              </button>
                            )
                          })}
                        </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        style={{ border: `1px solid rgba(255,255,255,0.15)`, minHeight: '220px' }}
                ...
                {/* Summary strip */}
                {(state.pieceType || state.metal || state.goldColor) && (
                  <motion.div
                    ...
                    <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm">
                      ...
                      {state.metal && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white font-semibold">{metalLabel}</span>
                        </div>
                      )}
                      {state.goldColor && (
                        <div className="flex items-center gap-2">
                          <span className="text-white/45">Colour</span>
                          <span className="text-white font-semibold">{goldLabel}</span>
                        </div>
                      )}
                    </div>
                  </motion.div>
